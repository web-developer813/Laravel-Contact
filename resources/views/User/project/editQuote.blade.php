@extends('user.layout')
    @section('body')
            ...
            <div class="row">
                            ...
                            <div class="row">
                                  <div class="col-md-6">
                                      ...
                                      <div class="col-md-6 text-right">
                                          <a href = "{{URL::route('user.project.addQuote', array($people->id,$project->id))}}"class="btn-u btn-brd forest-btn-change-project" type="button" style="color: white">Add</a>
                                      </div>
                            </div>
                         </div>
                        <div class="panel-body">
                            @foreach($quote as $key=>$value)
                                <div class="row margin-bottom-5">
                                    ...
                                    <div class="col-md-4">
                                        <p><a href="{{URL::route('user.project.deleteQuote',array($value->id,'project'))}}" style="color:red">Delete</a></p>
                                    </div>
                                </div>
                ...
                <!-- Project Management Part -->
                     <div class="row margin-bottom-20">
                        ...
                        <div class="col-md-12">
                            <form action ="{{URL::route('user.project.editProjectSection')}}" method="post" id="editProjectSectionDiv">
                                {!! Form::token() !!}
                      ...
                      <!-- Project Management Part End -->
                      <!-- Add Zone Part start-->
                        <div class="row margin-bottom-20">
                            <div class="col-md-12">
                                 <form action ="{{URL::route('user.project.addZones')}}" method="post" id="addProjectDiv">
                                    {!! Form::token() !!}
                                    <input type="hidden" name="projectId" value="{{$project->id}}">
                                       ...
                                       <div class="row margin-bottom-20">
                                           <div class="col-md-12 text-right">
                                                ...
                                                <input type="button" class="btn-u" value="Save Zones" onclick="onSaveZones()">
                                                <a href="{{URL::route('user.contact.main',$people->id)}}" class="btn-u btn-u-red">Cancel</a>
                                           </div>
                                       </div>
                                 </form>
                            </div>
                        ...
                        <div class="row margin-bottom-20">
                            ...
                            <div class="col-md-12"  id="zoneList">
                            <?php
                                $listProject ='';
                                for($i=0; $i<count($projectZone); $i++){
                                $list .='<p>';
                                if(strtoupper($projectZone[$i]->ProjectType->type) == "DIRECT SPACE"){
                                    $listProject .='<a href="javascript:void(0)" onclick="onChangeEditProject('.$projectZone[$i]->id.')">Edit</a> ';
                                    ...
                                    if($projectZone[$i]->AreaUnitId != ""){
                                     ...
                                     $listProject .= "   ".'<a href="'.URL::route('user.project.deleteZone', $projectZone[$i]->id).'" style="color:red">Delete</a>';
                                }else if(strtoupper($projectZone[$i]->ProjectType->type) == "In-Duct Duct Size"){
                                    $listProject .='<a href="javascript:void(0)" onChangeEditProject('.$projectZone[$i]->id.')"></a>';
                                    $listProject .='Zone '.($i+1).': ';
                                    $listProject .='<span>'.$projectZone[$i]->ProjectType->type.'</span> ';
        ...
        <!-- Edit Model for zone --->
        <div class="modal fade" id="editZoneModel" tabindex="-1" role="dialog"  aria-labelledby="basicModal" aria-hidden="true">
              <div class="modal-dialog">
                   <div class="modal-content modalChangeContent">
                       <div class="modal-header modalChangeHeader">
                           <button type="button" class="close" data-dismiss="modal" aria-hidden="true"></button>
                           <h4 class="modal-title modalChangeTitle" id="myModalLabel">Edit Zone</h4>
                       </div>
                       <div class="modal-body" id="zoneChangeContent">
                       </div>
                   </div>
              </div>
        </div>
        <!-- Edit note  model start--->
    ...
    @section('custom-scripts')
        ...
        <script type="text/javascript">
            ...
            function onChangeEditProject(id){
                var base_url = window.location.origin;
                   $.ajax ({
                      url: base_url + '/editZones',
                      type: 'POST',
                      data: {id: id, _token: $("input[name='_token']").val()},
                      cache: false,
                      dataType : "json",
                      success: function (data) {
                            $("#zoneChangeContent").html(data.list);
                            $("#editZoneModel").modal('show');
                      }
                   });
            }
            function onEditZoneStore(){
                $("#editZoneDiv").ajaxForm({
                    success:function(data){
                        if(data.result == "success"){
                            $("#editZoneModel").modal('hide');
                            $("#zoneList").html(data.list);
            ...
            function onSaveProject(){
                $("#editProjectSectionDiv").ajaxForm({
                    success:function(data){
                        if(data.result == "success"){
                            $("#editProjectSectionDiv").html(data.list);
                            bootbox.alert(data.message);
